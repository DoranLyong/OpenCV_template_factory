# code=<utf-8>
"""
* K-NN 알고리즘 구현하기.
* 트랙바로 k-value를 조정할 수 있게 만들기 
"""

import numpy as np 
import cv2 

from lib import mlutils

def on_k_changed(pos):
    global k_value

    k_value = pos
    if k_value < 1:
        k_value = 1

    trainAndDisplay()


def addPoint(x, y, c):
    train.append([x, y])
    label.append([c])


def trainAndDisplay():
    trainData = np.array(train, dtype=np.float32)
    labelData = np.array(label, dtype=np.int32)

    knn_model.train(trainData, cv2.ml.ROW_SAMPLE, labelData)

    h, w = img.shape[:2]
    for y in range(h):
        for x in range(w):
            sample = np.array([[x, y]]).astype(np.float32)

            ret, _, _, _ = knn_model.findNearest(sample, k_value)

            ret = int(ret)
            if ret == 0:
                img[y, x] = (128, 128, 255)
            elif ret == 1:
                img[y, x] = (128, 255, 128)
            elif ret == 2:
                img[y, x] = (255, 128, 128)

    for i in range(len(train)):
        x, y = train[i]
        l = label[i][0]

        if l == 0:
            cv2.circle(img, (x, y), 5, (0, 0, 128), -1, cv2.LINE_AA)
        elif l == 1:
            cv2.circle(img, (x, y), 5, (0, 128, 0), -1, cv2.LINE_AA)
        elif l == 2:
            cv2.circle(img, (x, y), 5, (128, 0, 0), -1, cv2.LINE_AA)

    cv2.imshow('knn', img)




    

""" 1. 학습 데이터 & 레이블  """
train = [] 
label = []

k_value = 1   # k-nn 
img = np.full((500, 500, 3), 255, np.uint8) # knn 알고리즘 결과를 보여줄 영상화면 
                                            # (h, w, c) = (500, 500, 3)인 초기화된 영상으로 생각하면 됨
knn_model = cv2.ml.KNearest_create() # 모델 초기화 


""" 2. 랜덤 데이터 생성 """
NUM = 30    # sample_data 개수 
rn = np.zeros((NUM, 2), np.int32)   # data_batch  
                                    # 데이터 벡터의 차원 수 = 2 
                                    # 데이터 개수 = NUM 
# (150, 150) 근방의 점은 0번 클래스로 설정
cv2.randn(rn, 0, 50)
for i in range(NUM):
    addPoint(rn[i, 0] + 150, rn[i, 1] + 150, 0)
# (350, 150) 근방의 점은 1번 클래스로 설정
cv2.randn(rn, 0, 50)
for i in range(NUM):
    addPoint(rn[i, 0] + 350, rn[i, 1] + 150, 1)
# (250, 400) 근방의 점은 2번 클래스로 설정
cv2.randn(rn, 0, 70)
for i in range(NUM):
    addPoint(rn[i, 0] + 250, rn[i, 1] + 400, 2)


""" 3. 영상 출력 창 생성 & 트랙바 생성 """
cv2.namedWindow('knn')
cv2.createTrackbar('k_value', 'knn', 1, 5, on_k_changed)


""" 4. KNN 결과 출력 """
trainAndDisplay()
cv2.waitKey()
cv2.destroyAllWindows()